...
# Run the MCP server
run:
	@echo "Running Physics MCP server..."
	@if command -v uv >/dev/null 2>&1; then \
		PYTHONPATH=src uv run python -m chuk_mcp_physics.server; \
	else \
		PYTHONPATH=src python3 -m chuk_mcp_physics.server; \
	fi
...
docker-build:
	...
	docker build -t chuk-mcp-physics:latest .
	@echo "Docker image built: chuk-mcp-physics:latest"

# Run Docker container
docker-run:
	@echo "Running Docker container..."
	docker run --rm -p 8000:8000 chuk-mcp-physics:latest
...
docker-stop:
	...
	@docker ps -q --filter "ancestor=chuk-mcp-physics:latest" | xargs -r docker stop

# Remove Docker image
docker-clean:
	@echo "Removing Docker image..."
	@docker rmi chuk-mcp-physics:latest 2>/dev/null || true
...
publish-test: build
	...
	echo ""; \
	echo "Install with:"; \
	echo "  pip install --index-url https://test.pypi.org/simple/ chuk-mcp-physics==$$version"
...
publish-manual: build
	...
	echo "Install with: pip install chuk-mcp-physics==$$version"
